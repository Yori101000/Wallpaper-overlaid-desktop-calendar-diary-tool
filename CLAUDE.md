...
| 今天 | **不在格子里** | 月历上方常驻的**今日块**；格子里只有数字 `FontWeights.Bold` 这一声 |
...
**「今天」不要再往格子里放记号。** 格子里曾经铺 `#21FFFFFF` 填充 + `#4DFFFFFF` 内描边，与 hover 层撞得分不清"今天"和"鼠标停在这格"。前后试过并被否掉十种形态：实心圆、细环圆、底部短横、圆角描边格、反白块、字号跃升、紧下划线、顶部游标、其余日期压暗、尖括号包夹 —— 结论是格子根本没有空间容纳它。识别任务已交给今日块（见下），格子里只留字重。

**待办不再有圆点。** 原先「有待办」同时给一枚 5px 琥珀圆点和一枚琥珀徽章，同色、同义，说了两遍。现在只留徽章（带数量，信息更多）。**副作用**：只有已完成待办的日子，格子里不再有任何痕迹 —— 信息仍在悬停提示与「待做」列表里。

### 今日块（`TodayPanel`）

`CalendarViewPanel` 里周头与月历之间那块（原 `TodayTodoPanel`）。它**常驻显示**，不再"有未完成待办才出现"—— 时有时无的话，"今天"就没有稳定的落点。
