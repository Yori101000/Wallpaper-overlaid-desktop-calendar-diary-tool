...
- **`SegmentButtonStyle`** —— 三个模式做成 tab：纯文字 + 选中项下方一条 2px 指示线（`ModeSelectedBrush`）。
...
| 今天 | **专属色 + 字号 + 字重** | `_todayColor` 天蓝（按文字色避让）+ 放大一档（`TodayNumberScale` 1.22）+ `Bold`；**优先级高于放假/调休** |
...
最终采用的是三重**非装饰**信号，且都不占新空间：**专属色 + 放大一档 + 加粗**，外加月历上方常驻的今日块。
注意"字号跃升"当初也在被否之列 —— 那是它作为**孤立标记**时被否的；现在它只是其中一重，且 `LineHeight` 恒按普通字号算，放大不会顶动农历行、不破坏整行基线。**只靠字重是不够的**：一屏 42 个数字里实测找不到。

配套的三条硬约束，改动前先读：

1. **颜色通道的优先级是 `今天 > 放假 > 调休 > 非本月 > 普通`**（`CalendarQuery.ResolveEmphasis`）。一个数字只能有一种颜色，今天恰好放假时以"今天"为准，法定属性由农历行后缀承担。
2. 因此 **`HolidaySuffix` 认的是 `holiday.IsOffDay` 而不是 `DayEmphasis`** —— 今天的 emphasis 已不携带法定属性，若还按 emphasis 取后缀，今天那天的「· 休」会凭空消失。
3. **今天那格不画徽章**：今日块已经写着「未完成 N 项」，格子里再挂一枚是同一件事说两遍，而且天蓝数字与琥珀徽章两个饱和色贴在一起会让那格发躁。
4. 今天也**不再往农历行写「今天」二字** —— 有了颜色之后那是第四重，而农历（初五）信息量更大。
...
摘要走 `CalendarQuery.BuildTodayPanelSummary`：**今天的未完成项优先，不足则补当月其余未完成项**（补位的带 `19日 · ` 前缀并压暗，否则会被读成今天的）。这是为了填掉一行化之后右侧剩下的横向空白。
